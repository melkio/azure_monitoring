...
try {
    ...
    $env:SUBSCRIPTION_ID = "<ENTER SUBSCRIPTION ID HERE>";
    $env:RESOURCE_GROUP = "<ENTER RESOURCE GROUP NAME HERE>";
    $env:TENANT_ID = "<ENTER TENANT ID HERE>";
    $env:LOCATION = "<ENTER LOCATION HERE>";
    $env:AUTH_TYPE = "principal";
    $env:CORRELATION_ID = "a2eb4ac3-b26b-4304-8c58-2d1fa98cf746";
    ...
    # Run connect command
    & "$env:ProgramW6432\AzureConnectedMachineAgent\azcmagent.exe" connect --service-principal-id "$ServicePrincipalId" --service-principal-secret "$ServicePrincipalClientSecret" --resource-group "$env:RESOURCE_GROUP" --tenant-id "$env:TENANT_ID" --location "$env:LOCATION" --subscription-id "$env:SUBSCRIPTION_ID" --cloud "$env:CLOUD" --tags 'Category=ND,Location=ND,Supplier=ND' --correlation-id "$env:CORRELATION_ID";
}
catch {
